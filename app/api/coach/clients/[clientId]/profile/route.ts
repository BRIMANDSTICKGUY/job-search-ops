...
async function getCoachScopedProfileTarget(req: Request, clientId: string) {
  ...
  if (clientError) {
    return { error: serverError(clientError.message || "Failed to load client") };
  }

  const authUserId = typeof clientRow?.auth_user_id === "string" ? clientRow.auth_user_id.trim() : "";
  if (!authUserId) {
    return { error: badRequest("Client is not linked to an auth user yet") };
  }
...
export async function GET(req: Request, { params }: { params: Promise<{ clientId: string }> }) {
  try {
    ...
    const auth = await getCoachScopedProfileTarget(req, clientId);
    if ("error" in auth) return auth.error;

    const { data, error } = await auth.supabase
      .from("client_profiles")
      .select("id, client_id, primary_role, secondary_role, career_level, core_skills, industry_keywords, preferred_locations, remote_preference, salary_min, salary_max, dealbreakers, created_at, updated_at")
...
export async function POST(req: Request, { params }: { params: Promise<{ clientId: string }> }) {
  try {
    ...
    if (!primaryRoleRaw) return badRequest("primary_role is required");
    if (!isRemotePreference(body.remote_preference)) return badRequest("remote_preference is required");
    ...
    if (existingError) return serverError(existingError.message || "Failed to load profile");
    ...
    if (error || !data) return serverError(error?.message || "Failed to create profile");
...
export async function PATCH(req: Request, { params }: { params: Promise<{ clientId: string }> }) {
  try {
    ...
    if (body.primary_role !== undefined && primaryRole === undefined) return badRequest("primary_role is invalid");
    if (body.career_level !== undefined && body.career_level !== null && !isCareerLevel(body.career_level)) {
      return badRequest("career_level is invalid");
    }
    ...
    if (error || !data) return serverError(error?.message || "Failed to update profile");
    return NextResponse.json({ ok: true, profile: data as ClientProfileRow });
  } catch (error) {
    return serverError(error instanceof Error ? error.message : "Unexpected server error");
  }
